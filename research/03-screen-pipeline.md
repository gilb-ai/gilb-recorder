...
|----|---------|----------|------|
| macOS 12.3+ | ScreenCaptureKit (`sck_rs`, persistent SCStream) | xcap | `screenpipe-screen/src/monitor.rs:142` |
| macOS <12.3 | xcap | — | — |
...
| `MAX_FPS` | 30 | в `screenpipe-core/src/video.rs:11` |
...
| OS | Primary | Fallback | Файл |
|----|---------|----------|------|
| macOS | Apple Vision (cidre VN) | Tesseract | `apple.rs:106-200` |
...
## 4. Оркестратор (screenpipe-engine, VisionManager)
...
- Native OS threads — только в `screenpipe-a11y` (event tap, Cocoa observer).
