...
### Два параллельных prior-artecorder
...
duration + Cues (seek index) в готовый .webm. prior-artecorder пишет WebM
...
`prior-artecorder` в renderer'е — это **не то, что мы хотим в gilb'е**.
...
- Альтернатива — `prior-art` (Pavel Tafintsev's bindings) или ручной
...
- НЕ копирует rodnik'овский Electron-prior-artecorder подход (это
...
- Pipeline записи (§3) **переписываем native'но**: Electron's
  prior-artecorder в renderer-окне — это рабочее, но архитектурно
