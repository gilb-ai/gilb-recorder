...
`prior-art-core/src/pipes/mod.rs:1-100`.

Pipe = директория `~/.prior-art/pipes/{name}/` с `pipe.md`:
...
`prior-art-redact/src/lib.rs:1-70`.
...
`prior-art-config/src/recording.rs` — `RecordingSettings` shared между
...
- CLI: TOML `~/.prior-art/config.toml`, default если нет.
- Desktop: `store.bin` через `tauri-plugin-store`.

**Env vars**:
- `prior-art_DATA_DIR` — override `~/.prior-art`
- `prior-art_FD_LIMIT=8192`
- `prior-art_ANALYTICS_ID`
...
```
~/.prior-art/             ($prior-art_DATA_DIR override)
├── db.sqlite
├── data/                  snapshots, video, audio
├── config.toml
├── pipes/{name}/pipe.md
├── logs/                  RollingFileAppender, daily rotation
└── secrets/               prior-art-secrets (keychain)
```
...
`prior-art-screen/src/core.rs` — `#[cfg(target_os = …)]` ветки выбирают
backend. `prior-art/src/platform/{macos,windows_uia,linux}.rs` —
...
8. **`~/.prior-art` layout** с `$prior-art_DATA_DIR` override — стандартный
