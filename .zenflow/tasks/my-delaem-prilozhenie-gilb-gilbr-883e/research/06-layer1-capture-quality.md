...
prior-art реализует все четыре. Ниже — конкретные механики, которые надо
...
Из `prior-art/src/events.rs:47-124` — полный список того, что
...
- если **Input Monitoring не дан** → CGEventTap не работает → теряем
  click/key/scroll. prior-art в этом режиме всё ещё пишет clipboard и app
...
Из VISION.md prior-art: `< 20% CPU, < 3 GB RAM`. Для Gilb v0 поставим
...
## 3. Robustness: что может пойти не так и как prior-art это переживает
...
- **Permission revoke detection** → publish `PermissionEvent::Lost`
  (`prior-art-events`), capture session переходит в reduced mode без
...
Триггеры синхронные. prior-art пробовал deferred indexer — откатил из-за
