# План: разбор архитектуры prior-art для Gilb
...
- [`research/02-storage.md`](research/02-storage.md) — `prior-art-db`:
...
### [x] Step: Разобрать структуру репозитория prior-art

Прочитал `CLAUDE.md`, `DESIGN.md`, корневые директории `apps/` и `crates/`.
Зафиксировано в `research/00-overview.md`.

### [x] Step: Глубокий разбор prior-art

Развёрнут Explore-agent на crate `prior-art`. Покрыто: API/деps,
...
### [x] Step: Глубокий разбор prior-art-db

Развёрнут Explore-agent на crate `prior-art-db`. Покрыто: SQLite + WAL,
...
- Глубокий разбор `prior-art-audio` (вне scope therblig'ов).
- `prior-art-vault` (encrypted storage) — посмотреть когда будем делать
  privacy-sensitive хранение.
- `prior-art-connect` (Slack/Obsidian) — релевантно когда дойдём до экспорта.
- `prior-art-apple-intelligence` — посмотреть когда будем интегрировать
  LLM-классификацию паттернов.
- Frontend паттерны (`apps/prior-art-app-tauri/`) — разобрать перед началом
  работы над UI Gilb.
