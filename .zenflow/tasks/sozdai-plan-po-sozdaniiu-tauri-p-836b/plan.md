...
- План пользователя "gilb — Tauri desktop app c full-spec БД как у prior-art" — Tauri shell, bundle ID `app.farol.gilb`, defaults `CAPTURE_EVENTS=1` / `CAPTURE_MOUSE_MOVE=0`.
...
- `gilb-a11y/src/platform/macos.rs`: CGEventTap LISTEN_ONLY через `prior-art`; IOHIDCheckAccess для permission.
...
- Миграция `0002_actions_fts.sql`: `actions_fts` virtual table (unicode61, external content) + AI/AU/AD triggers по паттерну prior-art `20260415000000_frames_fts_external_content.sql`.
