...
2. **План пользователя** "gilb — Tauri desktop app c full-spec БД как у prior-art" — описывает один Tauri проект + crate `native/capture-core`, полную multimodal схему БД (frames + elements + ocr_text + ui_events + audio + meetings + memories), фазы 0–8.
...
| FTS5 external content | full multimodal: frames_fts, elements_fts, ui_events_fts, audio_transcriptions_fts, memories_fts | один `actions_fts` (external content поверх `actions`) | **`actions_fts` в v0.** Остальные FTS-индексы добавляем вместе с соответствующими таблицами в более поздних фазах. Триггеры — синхронные (research §4.3: deferred indexer prior-art откатил из-за latency). |
...
- `actions_fts` — FTS5 external content поверх `actions(text_content, element_name, window_title, app_name)` + AI/AU/AD триггеры строго по паттерну prior-art `20260415000000_frames_fts_external_content.sql`.
...
- `gilb-a11y/src/platform/macos.rs`: CGEventTap LISTEN_ONLY через `prior-art`; IOHIDCheckAccess для permission check.
...
- Migration checksum self-heal (паттерн prior-art `db.rs:477-495`).
